...
ifeq ($(findstring --network OPSepolia,$(ARGS)),--network OPSepolia)
	NETWORK_ARGS := --rpc-url $(OPSEPOLIA_RPC_URL) --private-key $(PRIVATE_KEY) --broadcast --verify --etherscan-api-key $(ETHERSCAN_API_KEY) -vvvv
endif

deploy:
	@forge script script/DeployPriceGuess.s.sol:DeployPriceGuess $(NETWORK_ARGS)